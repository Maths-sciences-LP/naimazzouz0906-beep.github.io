...
\item $\theta = 268 - 273 = \mathbf{-5\ ^\circ\text{C}}$ (stockage au froid des colles vinyliques).
...
\item Le \textbf{thermocouple type K} : seul capteur de la liste capable de mesurer des températures aussi élevées (plage jusqu'à environ $1\,000\ ^\circ$C). La CTN ($\leq 150\ ^\circ$C) serait détruite, et la Pt100 ($\sim 850\ ^\circ$C) arriverait en limite de plage. Le thermocouple est bien adapté à la chambre de combustion d'une chaudière à copeaux.
\end{enumerate}
